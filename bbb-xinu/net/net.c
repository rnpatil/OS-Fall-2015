...
process	netin ()
{
	...
	/*kprintf("ETH_ARP %d",ETH_ARP)
	kprintf("ETH_IP %d",ETH_IP) */
	...
	while(1) {
		...
		//printf("pkt value %d",pkt);

		/* Obtain next packet that arrives */
 		//kprintf("ETHER0 %d",ETHER0);
  		//kprintf("Packet %d",pkt);
 		//kprintf("PACKLEN %d",(char *)pkt);
		...
		//kprintf("pkt->net_ethtype %d",pkt->net_ethtype);
